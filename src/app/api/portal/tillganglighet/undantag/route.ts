...
import { replaceAvailabilityException } from "@/lib/portal/availability";
import { isFixedPublicBlock } from "@/lib/portal/types";

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

/**
 * Carolina overrides one date: unavailable, or a custom set of fixed public
 * blocks. The whole state for that date is replaced atomically by a single
 * RPC, so "unavailable" and "custom" can never end up coexisting.
 */
export async function POST(request: Request) {
  ...
  // "unavailable" — it can never make a weekend publicly bookable. The RPC
  // enforces this too; this check just gives a better message.
  ...
  if (type === "custom") {
    ...
    for (const item of intervals) {
      ...
      if (blocks.some((block) => block.startTime === startTime)) {
        return Response.json({ ok: false, error: "Samma block angavs flera gånger." }, { status: 400 });
      }
      blocks.push({ startTime, endTime });
    }
  }

  const exceptions = await replaceAvailabilityException({ date, type, blocks });
  if (!exceptions) {
    return Response.json({ ok: false, error: "Kunde inte spara undantaget." }, { status: 502 });
  }
