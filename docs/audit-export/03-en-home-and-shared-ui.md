# 03 — English homepage and shared public UI

Repository: redrider81/forsa
Branch: main
Commit: 088ffd03c531b59dc2772714249af8fa87a50654

English homepage and the English message dictionary.
...
English routes reuse the same shared components as Swedish, selecting strings by locale.
There are no separate English navigation, footer, CTA, service-grid or process-grid
components. Their source appears once, in 01, 01a, 01b and 01c:

- `src/components/site-navigation.tsx` — 01b, 01c
- `src/components/site-footer.tsx` — 01
- `src/components/cta-link.tsx` — 01
- `src/components/coaching-services-grid.tsx` — 01a (contains `servicesSv` and `servicesEn`)
- `src/components/engagement-bento-grid.tsx` — 01a (contains `stepsSv` and `stepsEn`)
- `src/components/process-faq.tsx` — 02a (locale-agnostic; items supplied per page)

`src/components/ui/kinetic-team-hybrid.tsx` ("Coaching med Carolina") is Swedish-only and
is not rendered on `/en`.

## English public image references and alt text

| Asset | Referenced from | Alt text |
| --- | --- | --- |
| `/supertable.png` | `src/app/en/page.tsx` | `Preparing for a coaching session` |
| `/superoffice.png` | `src/app/en/page.tsx` | `Individual coaching at CVB Coaching` |
| `/supermeeting.png` | `src/app/en/page.tsx` | `Business coaching in a confidential session` |
| `/cvb-monogram.png`, `/cvb-logo.png` | `src/components/brand/logo.tsx` | `CVB Coaching` |
| `/cvb1.mp4` | `src/components/hero-video-background.tsx` | decorative background video, no alt |

The English homepage still references these three photographic assets; the Swedish
homepage references none.
